...
import { Button, Calendar, Card, CardContent, CardFooter, CardHeader, CardTitle, Popover, PopoverContent, PopoverTrigger, Separator, Skeleton, Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/shared/ui'
	...
	return (
		<>
			...
			<div className="grid grid-cols-2 gap-2">
				...
				{!isLoading && uniqueDays?.map((item: string, index) => 
					<Card className='w-[100%]' key={index}>
						<CardHeader className='space-y-2'>
							<CardTitle>{item} - {format(new Date(item), "EEEE", { locale: ru })} </CardTitle>
						</CardHeader>
						<CardContent>
							<Table>
								<TableHeader>
								...
								<TableBody>
								{schedule?.data?.schedules.filter((schedule:any) => schedule.day.includes(item)).map((item:any) => 
